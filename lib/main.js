require('preferences-service').set('extensions.checkCompatibility.nightly', false);
...
function save_visited_ID(new_id) {
  ...
  } else if (sStore.id_found && sStore.id_found.indexOf(new_id) === -1) {
    sStore.id_found += ' ' + new_id;
  }
  console.log(sStore.id_found);
  if (sStore.id_found.split(' ').length > 12) {
    saveIt_pubmeder( sStore.id_found.replace(' ', ',') );
  }
}
...
function saveIt_pubmeder(pmid) {
  ...
  req({
    ...
    onComplete: function (response) {
      var d = response.json;
      if (d && d.respond > 1) {
        console.log('all sent to www.pubmeder.com');
        if (sStore.id_history === undefined) {
          sStore.id_history = sStore.id_found;
        } else {
          sStore.id_history += ' ' + sStore.id_found;
        }
        sStore.id_found = '';
...
var base_uri = 'https://pubget-hrd.appspot.com',
  ...
  no_context_menu = prefSet.prefs.no_context_menu || 0,
  rev_proxy = 0;
...
exports.main = function(options, callbacks) {
  ...
  var menu_a = no_context_menu || contextMenu.Item({
    ...
    contentScript: 'var text = window.getSelection().toString();' +
                   'var new_title = "Search the_Paper_Link for " + text;' +
                   'if (text.length > 20) {' +
                   '  new_title = "Search the_Paper_Link for " + text.substr(0, 20) + "...";' +
                   '}' +
  ...
  var inject_b = pageMod.PageMod({
    ...
    onAttach: function onAttach(worker) {
      ...
      worker.on('message', function(msg) {
        console.log(msg);
        apikey = msg;
        sStore.thepaperlink_apikey = msg;
      });
      worker.on('detach', function () {
        detachWorker(this, workers);
      });
...
prefSet.on('ezproxy_prefix', function () {
  var pref_value = prefSet.prefs.ezproxy_prefix;
  if (pref_value && pref_value !== 'http://a.b.c/d?url='
    && (pref_value.substr(0,7) === 'http://' || pref_value.substr(0,8) === 'https://')) {
    ezproxy_prefix = pref_value;
    console.log('The ezproxy_prefix preference changed');
  } else {
    prefSet.prefs.ezproxy_prefix = 'http://a.b.c/d?url=';
    console.log('Not a valid ezproxy_prefix value');
  }
});
...
prefSet.on('no_context_menu', function () {
  if ( prefSet.prefs.no_context_menu ) {
    no_context_menu = 1;
    console.log('Will not shown any contextual menu');
  }
});
...
prefSet.on('rev_proxy', function () {
  if ( prefSet.prefs.rev_proxy ) {
    rev_proxy = 1;
    console.log('Will use the reverse proxied server');
  }
});