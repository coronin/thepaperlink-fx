...
function queue_scholar_title() {
  NO_DEBUG || console.log('not support in Firefox yet');
...
function do_scholar_title() {
  ...
  if (scholar_run === scholar_count) {
    ...
    scholar_run = 0;
    scholar_queue = [];
    NO_DEBUG || console.log('self-reset scholar_count _run _queue');
  }
}

function scholar_title(pmid, t) {
  NO_DEBUG || console.log('pmid', pmid);
  NO_DEBUG || console.log('title', t);
...
function parse_url(pmid, url) {
  NO_DEBUG || console.log('pmid', pmid);
  NO_DEBUG || console.log('url', url);
  var in_mem = sStore['url_' + pmid];
  if (in_mem) {
    in_mem = in_mem.split(',', 2);
    for (i = 0; i < pubmed_workers.length; i += 1) {
      pubmed_workers[i].postMessage(['el_link', '_pdf' + pmid, in_mem[1]]);
    }
    return;
  }
  for (i = 0; i < pubmed_workers.length; i += 1) {
    pubmed_workers[i].postMessage(['el_link', '_pdf' + pmid, 1]);
  }
  // chrome code not work here
}
...
exports.main = function(options, callbacks) {
    ...
    onAttach: function onAttach(worker) {
      ...
      worker.on('message', function(msg) {
        ...
        if (msg[0] === 'url') {
          ...
          scholar_queue = [];
          pubmed_workers = [];
          NO_DEBUG || console.log('on-request reset scholar_count _run _queue');
  ...
  var inject_b = pageMod.PageMod({
    ...
    contentScript: 'var d = document;' +
                   ...
                   'if (nod) {' +
                   '  nod.textContent = "your Firefox is all set for that";' +
                   '}' +
                   'if (a && c) {' +
