...
var ENTREZAJAX_APIKEY = '68ed3a6f270926237eaeeb2f7b2a7f27',
  ...
  ws_addr = sStore.ws_address || 'node.thepaperlink.com:8081',
  uid = sStore.ip_time_uid || null,
  local_ip;

function must_success_first() {
  req({
    url: 'http://node.thepaperlink.com:8089',
    onComplete: function (d) {
      if (d.status === 200) {
        local_ip = d.json['x-forwarded-for'];
        DEBUG && console.log('main 31, local_ip: ' + local_ip);
        if (local_ip && !uid) {
          uid = local_ip + ':';
          ...
          onComplete: function (dd) {
            ...
            if (dd.status === 200) {
              ...
              if (dd.json['fx'] && version != dd.json['fx']) {
                if (rev_proxy) {
                  tabs.open('http://www.zhaowenxian.com/static/about_us.html');
                } else {
...
function reload_about_us() {
  for each (var tab in tabs) {
    if (tab.url === 'http://www.thepaperlink.com/static/about_us.html' || tab.url === 'http://www.zhaowenxian.com/static/about_us.html')
      tab.reload();
  }
}
...
function open_tab_with_url(url) {
  if (rev_proxy) {
    tabs.open('http://www.zhaowenxian.com' + url);
  } else {
...
function detachWorker(worker, workerArray) {
  ...
  if(i !== -1) {
    ...
    DEBUG && console.log('main 42: worker detached');
  } else {
    DEBUG && console.log('main 44: worker remains');
  }
}
...
function update_cloud_op() {
  ...
  if (oauth_status.indexOf('baiduyun') > -1) {
    cloud_op += 'y';
  }
  DEBUG && console.log('main 72, cloud_op: ' + cloud_op);
}

function force_rev_proxy() {
  base_uri = 'http://www.zhaowenxian.com';
  rev_proxy = true;
  DEBUG && console.log('main 78: rev_proxy_check timeout, force rev_proxy');
}
  ...
  req({
    ...
    onComplete: function (response) {
      ...
      if (response.status !== 200) {
        base_uri = 'http://www.zhaowenxian.com';
        rev_proxy = true;
        DEBUG && console.log('main 90: ' + response.status + ', you are accessing the reverse proxied server, which is slower.');
      } else {
        DEBUG && console.log('main 92: Hi, you are accessing our secured server. Speedy!');
      }
    }
  }).get();
}
...
function save_visited_ID(new_id) {
  ...
  DEBUG && console.log('main 104: ' + sStore.id_found);
...
function saveIt_pubmeder(pmid) {
  if (pubmeder_apikey === null || pubmeder_apikey === null) {
    DEBUG && console.log('main 112: no valid pubmeder credit, open related URL');
  ...
  DEBUG && console.log('main 123, req : ' + url);
  req({
    url: url,
    onComplete: function (response) {
      var d = response.json;
      if (d && d.respond > 1) {
        DEBUG && console.log('main 129: all sent to www.pubmeder.com');
        if (sStore.id_history === undefined) {
          sStore.id_history = pmid;
        } else {
          sStore.id_history += ',' + pmid;
          DEBUG && console.log('main 134: ' + sStore.id_history);
        }
        sStore.id_found = '';
      }
    }
...
function update_ezproxy_prefix(p) {
  if (p && p !== 'http://a.b.c/d?url='
    ...
    ezproxy_prefix = p;
  } else if (p && p.substr(0,1) === '.' && p.substr(1,1) !== '.') {
    var ezproxy_endswith = p.substr(-3,3).toLowerCase();
    if (ezproxy_endswith === 'edu' || ezproxy_endswith === 'net' || ezproxy_endswith === 'com' || ezproxy_endswith === 'org' || ezproxy_endswith === 'gov') {
      ...
      DEBUG && console.log('main 153: not a valid ezproxy_prefix sub-domain, ignore');
    }
  } else {
    ezproxy_prefix = '';
    prefSet.prefs.ezproxy_prefix = 'http://a.b.c/d?url=';
    if (p !== 'http://a.b.c/d?url=') {
      DEBUG && console.log('main 159: not a valid ezproxy_prefix value, use ""');
    }
  }
}
...
function eSearch(search_term) {
  ...
  DEBUG && console.log('main 170, req : ' + url);
  req({
    url: url,
    onComplete: function (response) {
      if (response.json && response.json.result.IdList.length === 1) {
        ...
        DEBUG && console.log('main 177, eSearch:\n' + response.status);
      }
    }
  }).get();
}
...
function upload_pdf_as_binary(upload_url, pdf, pmid, apikey, no_email) {
  ...
  if (msg === null) {
    if (!no_email) {
      DEBUG && console.log('main 178, req : ' + base_uri);
      req({
        url: base_uri + '/',
        content : {'pmid': pmid, 'apikey': apikey, 'action': 'email'},
        onComplete: function (response) {
          DEBUG && console.log('main 193: pdf upload failed, but email sent');
        }
      }).post();
  } }
}
...
function do_scholar_title() {
  ...
  if (scholar_run === scholar_count) {
    ...
    scholar_run = 0;
    scholar_queue = [];
    DEBUG && console.log('main 215: self-reset scholar_count _run _queue');
  }
}
...
function scholar_title(pmid, t, to_workers) {
  DEBUG && console.log('main 230: pmid', pmid);
  DEBUG && console.log('main 231: title', t);
...
function parse_url(pmid, url, a_worker) {
  DEBUG && console.log('main 287: pmid', pmid);
  DEBUG && console.log('main 288: url', url);
  ...
  var r = loadHTML(url),
    ...
    args;
  if (h && h.length) {
    DEBUG && console.log('main 312: ' + h);
    args = {'apikey': apikey, 'pmid': pmid, 'pii_link': h[1]};
    if (h2 && h2.length) {
      DEBUG && console.log('main 315: ' + h2);
      args.scopus_n = h2[1];
...
var base_uri = 'https://pubget-hrd.appspot.com',
  ...
  guest_apikey = sStore.thepaperlink_guest_apikey || null,
...
if (last_date !== date_str) {
  ...
  if ( prefSet.prefs.tab_open_if_no_apikey ) {
    if (!apikey) {
      if (rev_proxy) {
        tabs.open('http://www.zhaowenxian.com/reg');
      } else {
    ...
    DEBUG && console.log('main 465: update date, but not alert; ' + date_str);
  }
}

exports.main = function(options, callbacks) {
  DEBUG && console.log('main 471: ' + options.loadReason);
  ...
  var menu_a = no_context_menu || contextMenu.Item({
    ...
    onMessage: function (item) {
      DEBUG && console.log('main 488: seaching with "' + item + '"');
  ...
  var menu_b = no_context_menu || contextMenu.Item({
    ...
    contentScript: 'self.on("click", function () {' +
                   ...
                   '});',
    onMessage: function (item) {
      DEBUG && console.log('main 502: visiting our service ' + item);
  ...
  var inject_a = pageMod.PageMod({
    ...
    onAttach: function onAttach(worker) {
      DEBUG && console.log('main 556: attaching content script for pubmed pages');
      ...
      worker.port.on('bar_msg', function(msg) {
        DEBUG && console.log('main 568: ' + msg);
        if (msg[0] === 't_cont') {
          DEBUG && console.log('main 593: ' + msg[1]);
          ...
          if (!req_key) {
            must_success_first();
            worker.port.emit('to_bar', ['wrong', 'No valid APIKEY']);
          } else {
            if (msg[1]) {
              DEBUG && console.log('main 572, req : ' + base_uri);
          ...
          scholar_run = 0;
          scholar_queue = [];
          DEBUG && console.log('main 603: on-request reset scholar_count _run _queue');
  ...
  var inject_b = pageMod.PageMod({
    ...
    contentScript: 'var d = document;' +
                   ...
                   '}' +
                   'if (a && c) {' +
                   '  self.port.emit("0.cail", [a.textContent, c.textContent]);' +
                   '}',
    onAttach: function onAttach(worker) {
      DEBUG && console.log('main 644: attaching content script for thepaperlink');
      //workers.push(worker);
      worker.port.on('0.cail', function(msg) {
        DEBUG && console.log('main 647: ' + msg);
        apikey = msg[0];
  ...
  var inject_c = pageMod.PageMod({
    ...
    contentScript: 'var d = document;' +
                   ...
                   'if (e && a) {' +
                   '  self.port.emit("1.cail", [e.textContent, a.textContent]);' +
                   '}',
    onAttach: function onAttach(worker) {
      DEBUG && console.log('main 676: attaching content script for pubmeder');
      //workers.push(worker);
      worker.port.on('1.cail', function(msg) {
        DEBUG && console.log('main 679: ' + msg);
  ...
  var inject_d = pageMod.PageMod({
    ...
    contentScript: 'var d = document;' +
                   ...
                   '}',
    onAttach: function onAttach(worker) {
      DEBUG && console.log('main 703: attaching content script for thepaperlink-oauth');
      //workers.push(worker);
      worker.port.on('pl_oauth', function(msg) {
        DEBUG && console.log('main 706: ' + msg);
        if (msg[0]) {
          var reload_var = 0;
  ...
  var inject_e = pageMod.PageMod({
    ...
    onAttach: function onAttach(worker) {
      DEBUG && console.log('main 733: attaching content script to found an ID');
      //workers.push(worker);
      worker.port.on('foundID', function(msg) {
        DEBUG && console.log('main 736: ' + msg);
        var dotCheck = /\./,
          pmcCheck = /PMC/;
  ...
  var inject_f = pageMod.PageMod({
    include: [
      'http://www.thepaperlink.com/static/about_us.html',
      'http://www.zhaowenxian.com/static/about_us.html'
      ],
    contentScriptWhen: 'ready',
    contentScriptFile: [ addon.url('jquery-1.8.3.min.js'),
                         addon.url('format_page.js') ],
    onAttach: function onAttach(worker) {
      DEBUG && console.log('main 760: attaching content script to instruction');
  ...
  var inject_g = pageMod.PageMod({
    ...
    onAttach: function onAttach(worker) {
      DEBUG && console.log('main 798: attaching content script 2 for thepaperlink');
      workers.push(worker);
...
prefSet.on('no_context_menu', function () {
  if ( prefSet.prefs.no_context_menu ) {
    ...
    DEBUG && console.log('main 817: will not shown any contextual menu');
    var i;
    for (i = 0; i < menus.length; i += 1) {
      menus[i].destroy();
    }
  } else {
    DEBUG && console.log('main 823: restart to show the contextual menu');
  }
});
...
prefSet.on('rev_proxy', function () {
  if ( prefSet.prefs.rev_proxy ) {
    rev_proxy = true;
    DEBUG && console.log('main 830: will use the reverse proxied server');
  } else {
    rev_proxy = false;
    DEBUG && console.log('main 833: will use the secured server');
  }
});
...
var ws = pageWorker.Page({
  ...
  onMessage: function (msg) {
    try {
      DEBUG && console.log('main 842: ' + msg);
      ...
      DEBUG && console.log('main 859, json parse error: ' + msg);
      return;
    }
  }
});