...
function scholar_title(pmid, t) {
  ...
  if (h && h.length) {
    ...
    if (g_num.length === 2 && g_link.length === 2) {
      ...
      for (i = 0; i < pubmed_workers.length; i += 1) {
        pubmed_workers[i].postMessage(['g_scholar', pmid, g_num[1], g_link[1]]);
      }
      req({
        url: base_uri + '/',
  ...
  for (i = 0; i < pubmed_workers.length; i += 1) {
    pubmed_workers[i].postMessage(['g_scholar', pmid, 0, 0]);
  }
...
function parse_url(pmid, url) {
  ...
  if (h && h.length) {
    ...
    if (h2 && h2.length) {
      ...
      for (i = 0; i < pubmed_workers.length; i += 1) {
        pubmed_workers[i].postMessage(['el_data', 'pl4_scopus' + pmid, h2[1]]);
      }
    }
    sStore['url_' + pmid] = pmid + ',' + h[1];
    for (i = 0; i < pubmed_workers.length; i += 1) {
      pubmed_workers[i].postMessage(['el_data', '_pdf' + pmid, h[1]]);
    }
    req({
      url: base_uri + '/',
      content : args,
  ...
  for (i = 0; i < pubmed_workers.length; i += 1) {
    pubmed_workers[i].postMessage(['el_data', '_pdf' + pmid, '://']);
  }
}
...
req({
  ...
  onComplete: function (d) {
    if (d.status === 200) {
      ...
      req({
        url: base_uri + '/',
        content : {'pmid':'1', 'title':'WEBSOCKET_SERVER', 'ip':local_ip},
